...
public class Bitmap {
	...
	protected static Logger logger = Logger.getLogger(RdpJPanel.class);
	...
	public static int[] bitmap_decompress(int width, int height, int size, RdpPacket_Localised data, int Bpp) throws RdesktopException{
		int[] ret=null;
		switch (Bpp)
		{
			case 1:
				ret = decompressInt(width, height, size, data, Bpp);
				break;
			case 2:
			case 3:
			case 4:
				ret = decompressInt3(width, height, size, data, Bpp);
				break;
			default:
				System.out.printf("Bpp %d\n", Bpp);
				break;
		}
		return ret;
	}
	public static int[] decompressInt3(int width, int height, int size, RdpPacket_Localised data, int Bpp) throws RdesktopException {
		...
		int[] pixel = new int[width * height * Bpp];
		while (input < end) {
			fom_mask = 0;
			code = (compressed_pixel[input++] & 0x000000ff);
			opcode = code >> 4;
